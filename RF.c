...
uchar API_Get_All_IntStatus(uchar IntStatusLength,uchar *IntStatus)
{
    ...
    API_WRITER[1]=0x00;//清除包处理器的所有中断
    API_WRITER[2]=0x00;//清除调制解调器的所有中断
    API_WRITER[3]=0x00;//清除芯片状态中断
...
void API_GPIO_CFG(uchar GPIO0,uchar GPIO1,uchar GPIO2,uchar GPIO3)
{
    ...
    API_WRITER[7]=0x00; //配置为输出的引脚的驱动能力设置
...
/*设置和读取寄存器*/
/**********************************************************************
寄存器的设置，对同一个组的多个寄存器连续进行设置（最多一次配置12个属性）
形参：group：寄存器所属的组
...
API_WRITER[0]代表设置参数命令
API_WRITER[1]代表该寄存器所属的组
API_WRITER[2]代表只写入一个特性参数*
API_WRITER[3]代表要写入的特性的地址/偏移量
API_WRITER[4]代表这个特性的值
如果写入成功，会读到CTS信号，否则认为失败
这样的写入并不保证会立即生效！！！只有在那个属性被用到的时候才会生效*/
...
/*获取芯片状态，传入参数是两个，0是射频的状态，1是射频的频段*/
